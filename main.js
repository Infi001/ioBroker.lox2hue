...
class Loxone2Hue extends utils.Adapter {
    ...
    async stateExists(id) {
        if (this.stateExistsCache.has(id)) return this.stateExistsCache.get(id);
    ...
    async initHueBridge() {
        ...
        } catch (e) { /* egal, dann eben nur die eigene Konfiguration */ }
        ...
        if (!this.hueBridgeHost) {
            throw new Error('Keine Hue-Bridge-IP bekannt - bitte "Hue-Bridge-IP" in der Adapter-Konfiguration eintragen.');
        }
        if (!this.hueBridgeUser) {
            throw new Error('Kein Hue-Bridge-API-Token gesetzt - bitte "Hue-Bridge-API-Token" in der Adapter-Konfiguration eintragen ' +
                '(zu finden in der Konfiguration der ' + this.config.hueInstance + '-Instanz, Feld "Nutzer"/"user" - dort geschützt, ' +
    ...
    async detectDevice(hueId, loxoneBaseId) {
        if (this.deviceCapabilities[hueId]) return this.deviceCapabilities[hueId];

        const isGroup = await this.stateExists(hueId + '.action.on');
        const cmdPrefix = isGroup ? '.action' : '';

        const hasXY = (await this.stateExists(hueId + cmdPrefix + '.xy')) || (await this.stateExists(hueId + cmdPrefix + '.hue'));
        ...
        this.log.debug('[INIT] ' + hueId + ' (' + (isGroup ? 'GRUPPE' : 'LAMPE') + '), Typ ' +
            (type.isRGB ? 'RGB' : (type.isDimmer ? 'Dimmer' : 'Switch')) + ', lauscht auf ' + type.loxoneSuffix);
        return type;
    }
    ...
    async computeCommand(inputVal, hueId) {
        ...
        if (cmd) this.lastKnownOn.set(hueId, !cmd.isOff);
        return cmd;
    }
    ...
    // Loxone-Ausgang trotzdem RGB liefert (Nutzerwunsch 28.08.): statt die Farbinfo
    // komplett zu ignorieren und immer nur die feste Farbtemperatur zu setzen, wird
    // versucht, den "Warmton" der gesendeten Farbe als Farbtemperatur nachzubilden.
    //
    ...
    berechneRohbefehl(inputVal, hueId) {
        ...
        if (typeof inputVal === 'string' && inputVal.includes(',')) {
            ...
            if (typeof inputVal === 'boolean') brightnessPercent = inputVal ? 100 : 0;
        ...
        if (type.isRGB && rgbString) {
            ...
            if (hueBri === 0) {
                return { groupable: true, isOff: true, writes: [{ suffix: '.command', value: JSON.stringify({ on: false, transitiontime: offFade }) }] };
            }
            const xy = this.rgbZuXy(r, g, b);

            return { groupable: true, isOff: false, writes: [{ suffix: '.command', value: JSON.stringify({ on: true, xy, level: hueBri, transitiontime: onFade }) }] };
        }

        // B) Dimmer / White Ambiance
        if (type.isDimmer) {
            if (brightnessPercent <= 0) {
                if (isTradfri) return { groupable: false, isOff: true, writes: [{ suffix: '.on', value: false }] };
                return { groupable: true, isOff: true, writes: [{ suffix: '.command', value: JSON.stringify({ on: false, transitiontime: offFade }) }] };
            }
            ...
            if (miredWert !== null) command.ct = miredWert;
            ...
            return { groupable: true, isOff: false, writes: [{ suffix: '.command', value: JSON.stringify(command) }] };
        }

        // C) Schalter
        if (type.isSwitch) {
            return { groupable: true, isOff: !switchState, writes: [{ suffix: '.on', value: switchState }] };
        }
    ...
    async applyCommand(hueId, prefix, cmd) {
        ...
        for (const w of cmd.writes) {
            if (w.delayMs) {
                setTimeout(() => this.setForeignStateAsync(hueId + prefix + w.suffix, w.value, false), w.delayMs);
            } else {
    ...
    scheduleAction(loxoneId, hueId, val) {
        ...
        if (!this.debounceTimer) {
            this.debounceTimer = setTimeout(() => this.executeBatch(), this.config.debounceMs);
        }
    }
    ...
    commandZuLightstate(cmd) {
        if (!cmd || !cmd.writes || !cmd.writes.length) return null;
        const cmdWrite = cmd.writes.find(w => w.suffix === '.command');
        if (cmdWrite) {
            let parsed;
            try { parsed = JSON.parse(cmdWrite.value); } catch (e) { return null; }
            const state = { on: !!parsed.on, transitiontime: parsed.transitiontime || 0 };
            if (typeof parsed.level === 'number') state.bri = Math.max(1, Math.round(parsed.level * 2.54));
            if (parsed.xy) state.xy = parsed.xy;
            if (parsed.ct) state.ct = parsed.ct;
            return state;
        }
        const onWrite = cmd.writes.find(w => w.suffix === '.on');
        if (onWrite) return { on: !!onWrite.value };
    ...
    async ladeSzenenCache() {
        ...
        await this.setObjectNotExistsAsync(id, {
            type: 'state',
            common: { name: 'Szenen-Cache (Sollwerte je Raum+Stimmung)', type: 'string', role: 'json', read: true, write: true, def: '{}' },
            native: {}
        });
    ...
    markiereDynamisch(roomKey, moodNum) {
        if (!this.sceneCache[roomKey]) this.sceneCache[roomKey] = {};
        ...
        this.log.info('[SZENE] ' + roomKey + ' Mood ' + moodNum + ' kommt seit über ' +
            (this.config.dynamicGiveUpMs / 1000) + 's nicht zur Ruhe -> als dynamisch markiert.');
    }

    planeSettleCheck(roomKey) {
        const room = this.roomGroups[roomKey];
        if (!room || !room.activeMoodsId) return;
        if (room.currentMoodNum !== undefined && room.currentMoodSince &&
        ...
        room.settleTimer = setTimeout(() => this.snapshotRoom(roomKey), this.config.settleMs);
    }
    ...
    async aktualisiereBridgeSzene(roomKey, moodNum, snap) {
        ...
        for (const hueId of Object.keys(snap)) {
            const type = this.deviceCapabilities[hueId];
            if (!type || !type.bridgeId) continue;
            const state = this.commandZuLightstate(snap[hueId]);
            if (!state) continue;
            lightIds.push(type.bridgeId);
            states[type.bridgeId] = state;
        }
        if (lightIds.length < this.config.sceneMinLights) return null;

        const alteSceneId = this.sceneCache[roomKey] && this.sceneCache[roomKey][moodNum] && this.sceneCache[roomKey][moodNum].bridgeSceneId;
        if (alteSceneId) {
            try { await this.hueBridgeRequest('DELETE', '/scenes/' + alteSceneId); } catch (e) { /* egal */ }
        }
        try {
            ...
            if (!sceneId) {
                this.log.error('[SZENE] Bridge-Szene für ' + roomKey + '/' + moodNum + ' nicht angelegt: ' + JSON.stringify(created));
                return null;
            }
            for (const bId of lightIds) {
                await this.hueBridgeRequest('PUT', '/scenes/' + sceneId + '/lightstates/' + bId, states[bId]);
            }
            this.log.info('[SZENE] Bridge-Szene für ' + roomKey + '/' + moodNum + ' angelegt (' + sceneId + ', ' + lightIds.length + ' Lampen).');
            return sceneId;
        } catch (e) {
    ...
    async snapshotRoom(roomKey) {
        const room = this.roomGroups[roomKey];
        if (!room || !room.activeMoodsId || !(await this.stateExists(room.activeMoodsId))) return;
        ...
        for (const member of room.members) {
            ...
            if (!type || !type.fullLoxoneId || !(await this.stateExists(type.fullLoxoneId))) continue;
            const val = await this.getForeignVal(type.fullLoxoneId);
            const cmd = await this.computeCommand(val, member.hueId);
            if (cmd) snap[member.hueId] = cmd;
        }
        const bridgeSceneId = await this.aktualisiereBridgeSzene(roomKey, moodNum, snap);
        if (!this.sceneCache[roomKey]) this.sceneCache[roomKey] = {};
        this.sceneCache[roomKey][moodNum] = { lights: snap, bridgeSceneId: bridgeSceneId || undefined };
        this.speichereSzenenCacheDebounced();
        this.log.info('[SZENE] ' + roomKey + ' Mood ' + moodNum + ' eingeschwungen, ' + Object.keys(snap).length + ' Lampen gespeichert' +
    ...
    async wendeEinzelnAn(lightsMap) {
        for (const hueId of Object.keys(lightsMap)) {
            const type = this.deviceCapabilities[hueId];
            if (type) await this.applyCommand(hueId, type.cmdPrefix, lightsMap[hueId]);
        }
    }
    ...
    async wendeSzeneSofortAn(roomKey, moodNum) {
        ...
        if (entry.bridgeSceneId) {
            this.log.info('[SZENE] ' + roomKey + ' Mood ' + moodNum + ' bekannt -> Bridge-Szene ' + entry.bridgeSceneId +
                ' abrufen (1 Aufruf statt ' + Object.keys(lightsMap).length + ').');
            try {
                await this.hueBridgeRequest('PUT', '/groups/0/action', { scene: entry.bridgeSceneId });
            } catch (e) {
                this.log.warn('[SZENE] Bridge-Szenen-Abruf fehlgeschlagen (' + e.message + ') -> Einzelbefehle als Rückfall.');
                await this.wendeEinzelnAn(lightsMap);
            }
            return;
        }
        this.log.debug('[SZENE] ' + roomKey + ' Mood ' + moodNum + ' bekannt -> ' + Object.keys(lightsMap).length + ' Lampen einzeln sofort setzen.');
        await this.wendeEinzelnAn(lightsMap);
    }
    ...
    async pruefeUndKorrigiereAus() {
        ...
        for (const roomKey of Object.keys(this.roomGroups)) {
            for (const member of this.roomGroups[roomKey].members) {
                ...
                if (!cmd || !cmd.isOff) continue;

                const onStateId = member.hueId + type.cmdPrefix + '.on';
                if (!(await this.stateExists(onStateId))) continue;
                geprueft++;
                ...
                this.log.info('[WÄCHTER] ' + member.hueId + ' sollte aus sein, ist aber noch an' +
                    (reachable === false ? ' (nicht erreichbar!)' : '') + ' -> sende Aus erneut.');
    ...
    async aktualisiereLoxoneKontext(devices, nurBekannteGeraete) {
        ...
        for (const d of devices) {
            if (!d.loxoneId || !d.hueId) continue;
            total++;
            ...
            if (!type && !nurBekannteGeraete) {
                try { type = await this.detectDevice(d.hueId, normalisierteLoxoneId); } catch (e) { /* Hue-Geraet evtl. gerade nicht erreichbar */ }
            }
            if (!type) continue;
    ...
    async onReady() {
        ...
        if (!this.config.devices || !this.config.devices.length) {
            this.log.warn('Keine Geräte konfiguriert - bitte in der Adapter-Konfiguration die Loxone→Hue-Zuordnung eintragen.');
        }
        ...
        const devices = (this.config.devices || [])
            ...
            .map(d => ({ ...d, loxoneId: d.loxoneId.replace(/\.(rgb|position|active)$/, '') }));

        // Raeume gruppieren
        for (const d of devices) {
            const roomKey = this.loxoneRoomKey(d.loxoneId);
            if (!this.roomGroups[roomKey]) {
                this.roomGroups[roomKey] = { members: [], activeMoodsId: undefined, currentMoodNum: undefined, currentMoodSince: undefined, settleTimer: null };
            }
        ...
        for (const roomKey of Object.keys(this.roomGroups)) {
            ...
            if (await this.stateExists(activeMoodsId)) {
                ...
                this.log.info('[SZENE] ' + roomKey + ': Lichtsteuerungsbaustein erkannt, ' + this.roomGroups[roomKey].members.length + ' Lampen.');
            }
        }

        this.watchdogInterval = this.setInterval(() => this.pruefeUndKorrigiereAus(), this.config.watchdogIntervalMin * 60000);

        this.setState('info.connection', true, true);
        this.log.info('loxone2hue bereit: ' + devices.length + ' Geräte in ' + Object.keys(this.roomGroups).length + ' Räumen.');
    }
    ...
    onUnload(callback) {
        try {
            ...
            if (this.cacheSaveTimer) clearTimeout(this.cacheSaveTimer);
            if (this.watchdogInterval) this.clearInterval(this.watchdogInterval);
            for (const roomKey of Object.keys(this.roomGroups)) {
                if (this.roomGroups[roomKey].settleTimer) clearTimeout(this.roomGroups[roomKey].settleTimer);
            }
            callback();
        } catch (e) {
            callback();
